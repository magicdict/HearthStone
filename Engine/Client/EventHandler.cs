...
namespace Engine.Client
{
    public class EventHandler
    {
        ...
        /// </summary>
        /// <returns></returns>
        public List<String> 事件处理(ClientPlayerInfo game)
        {
            ...
            for (int j = 0; j < 事件池.Count; j++)
            {
                ...
                for (int i = 0; i < game.BasicInfo.BattleField.MinionCount; i++)
                {
                    ...
                    {
                        Result.AddRange(game.BasicInfo.BattleField.BattleMinions[i].事件处理方法(事件, game));
                    }
                }
        ...
        public delegate void delegate事件处理(ClientPlayerInfo game);
