            ...
            switch (card.Condition)
            {
                case SecretCondition.对方召唤随从:
                    ...
                    if (HitMySelf)
                    {
                        ...
                        //例如：亡语的时候可能召唤一个新的随从
                        game.YourInfo.BattleField.BattleMinions[int.Parse(actionField[3])].实际生命值 = 1;
                        ActionLst.Add(Card.Server.ActionCode.strPoint + Card.CardUtility.strSplitMark + CardUtility.strYou + Card.CardUtility.strSplitMark +
                                    actionField[3] + Card.CardUtility.strSplitMark + "1/" + game.YourInfo.BattleField.BattleMinions[int.Parse(actionField[3])].实际攻击力);
                    }
                    else
                    {
                        //在自己的回合运行别人的奥秘
                        ...
                        {
                            //SUMMON#YOU#M000001#POS
                            game.MySelf.RoleInfo.BattleField.BattleMinions[int.Parse(actionField[3])].实际生命值 = 1;
                            ActionLst.Add(Card.Server.ActionCode.strPoint + Card.CardUtility.strSplitMark + CardUtility.strMe + Card.CardUtility.strSplitMark +
                                    actionField[3] + Card.CardUtility.strSplitMark + "1/" + game.MySelf.RoleInfo.BattleField.BattleMinions[int.Parse(actionField[3])].实际攻击力);
                        }
                        else
                        {
                            //MINION#M000001#1
                            game.MySelf.RoleInfo.BattleField.BattleMinions[int.Parse(actionField[2])].实际生命值 = 1;
                            ActionLst.Add(Card.Server.ActionCode.strPoint + Card.CardUtility.strSplitMark + CardUtility.strMe + Card.CardUtility.strSplitMark +
                                    actionField[2] + Card.CardUtility.strSplitMark + "1/" + game.MySelf.RoleInfo.BattleField.BattleMinions[int.Parse(actionField[2])].实际攻击力);
                        }
                    }
                    break;
                default:
